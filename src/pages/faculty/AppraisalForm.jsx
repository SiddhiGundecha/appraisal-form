...
const removeTeachingRow = (index) => {
  ...
  localStorage.setItem("facultyDraft", JSON.stringify(draftData));
  alert("Draft saved successfully");
};
useEffect(() => {
  const draft = localStorage.getItem("facultyDraft");

  if (draft) {
    const data = JSON.parse(draft);

    setGeneralInfo(data.generalInfo || {});
    setTeachingActivities(data.teachingActivities || []);
    setSppuInvolvement(data.sppuInvolvement || {});
    setStudentFeedback(data.studentFeedback || []);
    setCategoryTwo(data.categoryTwo || []);
    setDepartmentalActivities(data.departmentalActivities || []);
    setInstituteActivities(data.instituteActivities || []);
    setSocietyActivities(data.societyActivities || []);
    setAcrDetails(data.acrDetails || {});
